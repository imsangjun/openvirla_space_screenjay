...
export function Campaign() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (scrollLeft + clientWidth >= scrollWidth - 10) { scrollContainer.scrollLeft = 0; }
      else if (scrollLeft <= 10) { scrollContainer.scrollLeft = scrollWidth / 2; }
    };
    scrollContainer.addEventListener('scroll', handleScroll);
    return () => scrollContainer.removeEventListener('scroll', handleScroll);
  }, []);

  const handleCardClick = (offer: CampaignOffer) => { setSelectedCampaign(offer); setIsDialogOpen(true); };
  const handleApply = () => {
    setIsDialogOpen(false);
    if (isLoggedIn) { setIsSuccessDialogOpen(true); } else { setIsLoginPromptOpen(true); }
  };
  const handleGoToLogin = () => { setIsLoginPromptOpen(false); navigate("/login"); };
  const handleGoToSignup = () => { setIsLoginPromptOpen(false); navigate("/login?mode=signup"); };

  const categories = ["All", "Product", "Offline"];
  const platforms = ["All", "Instagram", "TikTok", "YouTube"];

  let filteredCampaigns = campaigns;
  if (selectedCategory !== "All") filteredCampaigns = filteredCampaigns.filter(o => o.type === selectedCategory);
  if (selectedPlatform !== "All") filteredCampaigns = filteredCampaigns.filter(o => o.platforms.some(p => p.toLowerCase() === selectedPlatform.toLowerCase()));
  if (searchKeyword.trim() !== "") filteredCampaigns = filteredCampaigns.filter(o => o.title.toLowerCase().includes(searchKeyword.toLowerCase()) || o.company.toLowerCase().includes(searchKeyword.toLowerCase()) || o.description.toLowerCase().includes(searchKeyword.toLowerCase()));

  return (
    <div className="bg-white min-h-screen">
      <section className="py-16 bg-[#f9fafb00]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-2">
            <h1 className="font-bold text-gray-900 text-[50px]">Campaign Offers<span className="text-[#004DF6]">.</span></h1>
            <button onClick={() => setIsLoggedIn(!isLoggedIn)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${isLoggedIn ? 'bg-green-100 text-green-700 border border-green-300' : 'bg-gray-100 text-gray-700 border border-gray-300'}`}>
              {isLoggedIn ? '✓ Logged In' : 'Not Logged In'} (Demo)
            </button>
          </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="bg-gradient-to-br from-[#f0f4ff] to-[#e6f0ff] rounded-3xl p-8 shadow-[4px_4px_12px_rgba(0,77,246,0.12),-4px_-4px_12px_rgba(255,255,255,0.9),inset_-4px_-4px_16px_rgba(0,77,246,0.15),inset_4px_4px_16px_rgba(255,255,255,1)]">
              <h2 className="text-4xl font-bold text-gray-900 flex items-center gap-2 mb-6">
                <span className="inline-block w-2 h-2 bg-[#004DF6] rounded-full animate-pulse"></span>
                          ...
                          <div className="relative h-40 rounded-t-md overflow-hidden">
                            ...
                            <div className="absolute top-2 left-2 flex gap-1.5">
                              {offer.platforms.map((platform) => (
                                <div key={platform} className="w-6 h-6 bg-white/20 backdrop-blur-md rounded flex items-center justify-center text-white border border-white/30 shadow-lg">
          ...
          <div className="mb-6">
            ...
            <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  {categories.map((category) => (
                    <button key={category} onClick={() => setSelectedCategory(category)}
                      className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${selectedCategory === category ? 'bg-[#004DF6] text-white shadow-[2px_2px_8px_rgba(0,77,246,0.3),-2px_-2px_8px_rgba(128,167,251,0.3)]' : 'bg-white/60 text-gray-700 border border-gray-200 hover:bg-white/80 hover:border-[#004DF6]/30 shadow-[inset_2px_2px_6px_rgba(0,0,0,0.08),inset_-2px_-2px_6px_rgba(255,255,255,0.9),2px_2px_4px_rgba(0,0,0,0.06),-2px_-2px_4px_rgba(255,255,255,0.5)]'}`}>
                      {category}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex gap-2">
                  {platforms.map((platform) => (
                    <button key={platform} onClick={() => setSelectedPlatform(platform)}
                      className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${selectedPlatform === platform ? 'bg-[#004DF6] text-white shadow-[2px_2px_8px_rgba(0,77,246,0.3),-2px_-2px_8px_rgba(128,167,251,0.3)]' : 'bg-white/60 text-gray-700 border border-gray-200 hover:bg-white/80 hover:border-[#004DF6]/30 shadow-[inset_2px_2px_6px_rgba(0,0,0,0.08),inset_-2px_-2px_6px_rgba(255,255,255,0.9),2px_2px_4px_rgba(0,0,0,0.06),-2px_-2px_4px_rgba(255,255,255,0.5)]'}`}>
                      {platform}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <input type="text" placeholder="Search by title, company, or description..." value={searchKeyword} onChange={(e) => setSearchKeyword(e.target.value)}
          ...
          <div className="grid md:grid-cols-3 lg:grid-cols-4 gap-6">
            {filteredCampaigns.map((offer) => (
              <div key={offer.id} onClick={() => handleCardClick(offer)}
                className="bg-white/40 backdrop-blur-lg rounded-md border border-white/60 overflow-hidden hover:shadow-[0_20px_50px_rgba(0,77,246,0.35)] hover:bg-white/60 hover:scale-105 transition-all duration-300 group cursor-pointer shadow-lg">
                <div className="relative h-40 rounded-t-md overflow-hidden">
                  <ImageWithFallback src={offer.image} alt={offer.title} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent" />
                  <div className="absolute top-2 left-2 flex gap-1.5">
                ...
                <div className="flex flex-wrap gap-4 items-center">
                  <div className="flex items-center gap-2">
                    {selectedCampaign.platforms.map((platform) => (
                      <div key={platform} className="w-8 h-8 bg-[#e6f0ff] rounded flex items-center justify-center text-[#004DF6]">
                        <PlatformIcon platform={platform} />
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {selectedCampaign.deliverables.map((item, index) => (
                      <li key={index} className="flex items-start gap-2 text-sm text-gray-600">
                        <CheckCircle className="w-4 h-4 text-[#004DF6] flex-shrink-0 mt-0.5" />
                        <span>{item}</span>
                      </li>
                    ))}
                  </ul>
      ...
      <Dialog open={isLoginPromptOpen} onOpenChange={setIsLoginPromptOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="flex flex-col items-center justify-center text-center py-8 px-4">
            ...
            <button onClick={handleGoToLogin} className="w-full mb-3 px-8 py-3 bg-[#004DF6] rounded-xl text-white font-medium hover:bg-[#0041cc] transition-all">Log In</button>
            <button onClick={handleGoToSignup} className="w-full px-8 py-3 bg-gray-200 rounded-xl text-gray-700 font-medium hover:bg-gray-300 transition-all">Sign Up</button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
